...
class HXResponseHandler(ResponseHandler):
            ...
            for i in range(len(tagged_unfinished)):
                ...
                if len(match) > 0:
                    ...
                    if tagged_unfinished['bidvol'].iloc[i] != ratio * match[u'成交数量'].iloc[0]:
                        ...
                        changed = 1
                    if tagged_unfinished['withdraw'].iloc[i] != match[u'撤消数量'].iloc[0]:
                        tagged_unfinished['withdraw'].iloc[i] = match[u'撤消数量'].iloc[0]
                        changed = 1
                    if tagged_unfinished['status'].iloc[i] != self.status[match[u'备注'].iloc[0]]:
                        tagged_unfinished['status'].iloc[i] = self.status[match[u'备注'].iloc[0]]
                        changed = 1
        ...
        try:
            ...
            for i in range(len(untagged)):
                ...
                match = time_match[(time_match[u'证券代码'] == int(untagged['stockcode'].iloc[i])) &
                                   ...
                                   #(time_match[u'操作'] == (u'买入' if int(untagged['askvol'].iloc[i]) > 0 else u'卖出')) &
                                   (time_match[u'操作'] == (u'证券买入' if int(untagged['askvol'].iloc[i]) > 0 else u'证券卖出')) &
                                   (time_match['tagged'] == 0)]
                ...
                if len(match) > 0:
                    ...
                    if untagged['withdraw'].iloc[i] != match[u'撤消数量'].iloc[0]:
                        untagged['withdraw'].iloc[i] = match[u'撤消数量'].iloc[0]
                    if untagged['status'].iloc[i] != self.status[match[u'备注'].iloc[0]]:
                        untagged['status'].iloc[i] = self.status[match[u'备注'].iloc[0]]
